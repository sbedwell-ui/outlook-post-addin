...
`,styles:(e,t)=>sn`
  ...
  `});function ea(e){return e.purpose.trim().length>0&&e.outcomes.trim().length>0&&e.structure.trim().length>0&&e.timing.trim().length>0}function ta(){return new Promise((e,t)=>{const n=Office.context.mailbox.item;if(!n)return void t(new Error("No active item"));let r=0,o=0,i=0;const s=()=>{i++,2===i&&e(r+o)};n.requiredAttendees.getAsync(e=>{e.status===Office.AsyncResultStatus.Succeeded&&(r=e.value.length),s()}),n.optionalAttendees.getAsync(e=>{e.status===Office.AsyncResultStatus.Succeeded&&(o=e.value.length),s()})})}function na(){return new Promise((e,t)=>{const n=Office.context.mailbox.item;if(!n)return void t(new Error("No active item"));let r=null,o=null,i=0;const s=()=>{if(i++,2===i)if(r&&o){const t=o.getTime()-r.getTime(),n=Math.round(t/6e4);e(n)}else e(0)};n.start.getAsync(e=>{e.status===Office.AsyncResultStatus.Succeeded&&(r=e.value),s()}),n.end.getAsync(e=>{e.status===Office.AsyncResultStatus.Succeeded&&(o=e.value),s()})})}function ra(e){return e.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}var oa=n(72),ia=n.n(oa),sa=n(825),aa=n.n(sa),la=n(659),ca=n.n(la),ua=n(56),ha=n.n(ua),da=n(540),fa=n.n(da),pa=n(113),ga=n.n(pa),va=n(942),ma={};let ba,ya,Aa,wa,xa,Ca,$a,Ea,ka,Fa,Va,Ba,Sa,Da,Ta,Oa,Ra;async function La(){try{const n=(e=await ta())<=7?{message:`${e} attendee${1!==e?"s":""} - Good size`,isWarning:!1}:{message:`${e} attendees - Consider reducing to 5-7`,isWarning:!0};ya.textContent=n.message,n.isWarning?(ba.classList.add("warning"),ba.classList.remove("success")):ba.classList.remove("warning");const r=(t=await na())<=30||t<=45?{message:`${t} min`,suggestion:null}:{message:`${t} min`,suggestion:"Can this be 30 or 45 min?"};Aa.textContent=r.message,r.suggestion?(wa.textContent=r.suggestion,wa.classList.remove("hidden"),xa.classList.remove("hidden")):(wa.classList.add("hidden"),xa.classList.add("hidden")),await async function(){try{const e=await new Promise((e,t)=>{const n=Office.context.mailbox.item;if(!n)return void t(new Error("No active item"));let r=null,o=null,i=0;const s=()=>{i++,2===i&&(r&&o?e({start:r,end:o}):t(new Error("Could not get meeting times")))};n.start.getAsync(e=>{e.status===Office.AsyncResultStatus.Succeeded&&(r=e.value),s()}),n.end.getAsync(e=>{e.status===Office.AsyncResultStatus.Succeeded&&(o=e.value),s()})}),t=await na(),n=`${ra(e.start)} - ${ra(e.end)} (${t} min)`;Va.value&&!Va.value.includes(" - ")||(Va.value=n)}catch(e){console.error("Error updating timing field:",e)}}()}catch(e){console.error("Error refreshing meeting info:",e),ya.textContent="Unable to load",Aa.textContent="Unable to load"}var e,t}async function Pa(e){try{await(t=e,new Promise((e,n)=>{const r=Office.context.mailbox.item;r?r.start.getAsync(o=>{if(o.status===Office.AsyncResultStatus.Failed)return void n(o.error);const i=o.value,s=new Date(i.getTime()+60*t*1e3);r.end.setAsync(s,t=>{t.status===Office.AsyncResultStatus.Succeeded?e():n(t.error)})}):n(new Error("No active item"))})),Ha(`Duration set to ${e} minutes`,"success"),await La()}catch(e){console.error("Error setting duration:",e),Ha("Failed to update duration","error")}var t}function Ia(){const e={purpose:Ea.value,outcomes:ka.value,structure:Fa.value,timing:Va.value,isComplete:!1,lastUpdated:(new Date).toISOString()};return e.isComplete=ea(e),e}async function Na(e=!1){try{const t=Ia();await function(e){return new Promise((t,n)=>{Office.context.mailbox.item?.loadCustomPropertiesAsync(r=>{if(r.status===Office.AsyncResultStatus.Failed)return void n(r.error);const o=r.value;o.set("postData",JSON.stringify(e)),o.set("postComplete",String(e.isComplete)),o.saveAsync(e=>{e.status===Office.AsyncResultStatus.Succeeded?t():n(e.error)})})})}(t),e||Ha("Saved successfully","success"),function(){const e=function(e){return{purpose:e.purpose.trim().length>0,outcomes:e.outcomes.trim().length>0,structure:e.structure.trim().length>0,timing:e.timing.trim().length>0}}(Ia());Ma(Ea,e.purpose),Ma(ka,e.outcomes),Ma(Fa,e.structure),Ma(Va,e.timing)}()}catch(t){console.error("Error saving:",t),e||Ha("Failed to save","error")}}function Ma(e,t){const n=e.value?.trim();n?t?(e.classList.add("valid"),e.classList.remove("invalid")):(e.classList.add("invalid"),e.classList.remove("valid")):e.classList.remove("valid","invalid")}function za(e){Office.context.mailbox.item?.body.setAsync(e,{coercionType:Office.CoercionType.Html},e=>{e.status===Office.AsyncResultStatus.Succeeded?Ha("POST inserted into meeting body","success"):Ha("Failed to update meeting body","error")})}function ja(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}function Ha(e,t="info"){Ra.textContent=e,Ra.className=`status ${t}`,Ra.classList.remove("hidden"),setTimeout(()=>{Ra.classList.add("hidden")},3e3)}ma.styleTagTransform=ga(),ma.setAttributes=ha(),ma.insert=ca().bind(null,"head"),ma.domAPI=aa(),ma.insertStyleElement=fa(),ia()(va.A,ma),va.A&&va.A.locals&&va.A.locals,at.getOrCreate(undefined).withPrefix("fluent").register(ks(),qs(),Xs(),Js()),Office.onReady(async()=>{ba=document.getElementById("attendee-badge"),ya=document.getElementById("attendee-count"),Aa=document.getElementById("duration-text"),wa=document.getElementById("duration-suggestion"),xa=document.getElementById("duration-buttons"),Ca=document.getElementById("btn-30min"),$a=document.getElementById("btn-45min"),Ea=document.getElementById("purpose-field"),ka=document.getElementById("outcomes-field"),Fa=document.getElementById("structure-field"),Va=document.getElementById("timing-field"),Ba=document.getElementById("btn-save"),Sa=document.getElementById("btn-check"),Da=document.getElementById("btn-insert"),Ta=document.getElementById("validation-results"),Oa=document.getElementById("validation-content"),Ra=document.getElementById("status-message"),function(){let e;Ca.addEventListener("click",()=>Pa(30)),$a.addEventListener("click",()=>Pa(45)),Ba.addEventListener("click",()=>Na()),Sa.addEventListener("click",()=>async function(){try{await Na(!0),function(e){Ta.classList.remove("hidden","success","warning","error");let t="";e.isValid&&0===e.warnings.length?(Ta.classList.add("success"),t='<div class="success-item">All checks passed! Meeting is ready to send.</div>'):(e.errors.length>0&&(Ta.classList.add("error"),e.errors.forEach(e=>{t+=`<div class="error-item">&#x274C; ${e}</div>`})),e.warnings.length>0&&(0===e.errors.length&&Ta.classList.add("warning"),e.warnings.forEach(e=>{t+=`<div class="warning-item">&#x26A0; ${e}</div>`}))),Oa.innerHTML=t}(await async function(){const e={isValid:!0,warnings:[],errors:[]};try{const t=await ta();t>7&&e.warnings.push(`This meeting has ${t} attendees. Optimal size is 5-7.`);const n=await na();n>45&&e.warnings.push(`Consider shortening this ${n} min meeting to 30 or 45 min.`);const r=await new Promise((e,t)=>{Office.context.mailbox.item?.loadCustomPropertiesAsync(n=>{if(n.status===Office.AsyncResultStatus.Failed)return void t(n.error);const r=n.value;e("true"===r.get("postComplete"))})});r||(e.errors.push("Meeting is missing POST framework.\n\nTo fix: Click 'Don't Send', then open 'POST Meeting v3' from the ribbon (or '...' menu). Fill in Purpose, Outcomes, Structure, and Timing, then click 'Insert POST into Body'."),e.isValid=!1)}catch(t){console.error("Validation error:",t),e.warnings.push("Could not complete all validation checks.")}return e}())}catch(e){console.error("Error checking meeting:",e),Ha("Failed to validate meeting","error")}}()),Da.addEventListener("click",()=>async function(){try{await Na(!0);const e=Ia();if(!ea(e))return void Ha("Please complete all POST fields before inserting","error");const t=function(e){const t=e.outcomes.split("\n").filter(e=>e.trim()).map(e=>`<li>${ja(e.replace(/^[-*•]\s*/,""))}</li>`).join(""),n=e.structure.split("\n").filter(e=>e.trim()).map(e=>`<li>${ja(e.replace(/^\d+[.)]\s*/,""))}</li>`).join("");return`\n<div style="font-family: Segoe UI, sans-serif; padding: 16px; background: #f5f5f5; border-radius: 8px; margin-bottom: 16px;">\n  <h2 style="color: #0078d4; margin-top: 0; margin-bottom: 16px;">&#128203; MEETING POST</h2>\n\n  <h3 style="color: #323130; margin-bottom: 8px;">PURPOSE</h3>\n  <p style="margin-top: 0; margin-bottom: 16px;">${ja(e.purpose)}</p>\n\n  <h3 style="color: #323130; margin-bottom: 8px;">OUTCOMES</h3>\n  <ul style="margin-top: 0; margin-bottom: 16px; padding-left: 20px;">\n    ${t||"<li>No outcomes specified</li>"}\n  </ul>\n\n  <h3 style="color: #323130; margin-bottom: 8px;">STRUCTURE</h3>\n  <ol style="margin-top: 0; margin-bottom: 16px; padding-left: 20px;">\n    ${n||"<li>No structure specified</li>"}\n  </ol>\n\n  <h3 style="color: #323130; margin-bottom: 8px;">TIMING</h3>\n  <p style="margin-top: 0; margin-bottom: 0;">${ja(e.timing)}</p>\n</div>`.trim()}(e);Office.context.mailbox.item?.body.getAsync(Office.CoercionType.Html,e=>{if(e.status===Office.AsyncResultStatus.Failed)return void Ha("Failed to read meeting body","error");const n=e.value||"";n.includes("MEETING POST")?za(n.replace(/<div[^>]*>[\s\S]*?MEETING POST[\s\S]*?<\/div>\s*<\/div>/i,t)):za(t+"<br/><br/>"+n)})}catch(e){console.error("Error inserting POST:",e),Ha("Failed to insert POST into body","error")}}());const t=()=>{clearTimeout(e),e=window.setTimeout(()=>{Na(!0)},1e3)};Ea.addEventListener("input",t),ka.addEventListener("input",t),Fa.addEventListener("input",t),Va.addEventListener("input",t)}(),await La(),await async function(){try{const e=await new Promise((e,t)=>{Office.context.mailbox.item?.loadCustomPropertiesAsync(n=>{if(n.status===Office.AsyncResultStatus.Failed)return void t(n.error);const r=n.value.get("postData");if(r)try{e(JSON.parse(r))}catch{e(null)}else e(null)})});e&&(Ea.value=e.purpose,ka.value=e.outcomes,Fa.value=e.structure,e.timing&&(Va.value=e.timing))}catch(e){console.error("Error loading form data:",e)}}()})})();
